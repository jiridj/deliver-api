...
describe('test user model', () => {
  it('should find a user by their email address', async () => {
    const user = await User.findByEmail('vhuett1@imgur.com');
    expect(user.first_name).toBe('Vin');
  });
  ...
  it('should check the user privileges', async () => {
    ...
    const admin = await User.findByEmail('admin@deliver.api');
  ...
  it('should check the user password', async () => {
    ...
    valid = await user.isValidPassword('otEfi7A1m');
