...
const secret = process.env.DELIVER_API_SECRET_KEY;
...
module.exports = class AuthController {
        ...
        async (req, email, password, done) => {
          try {
            ...
            if (!user) {
              ...
              return done(AppError.badRequest('Account with email exists'));
          } catch (err) {
            logger.error(err);
            return done(err);
          }
  ...
  async requestPasswordReset(req, res) {
    ...
    logger.debug(user);
    user.one_time_password = Math.floor(Math.random() * 9000 + 1000).toString();
    logger.debug(user.one_time_password);
    await user.save();
    res.status(200).send();
  }
  ...
  async validatePasswordReset(req, res, next) {
    ...
    if (valid) {
      ...
      await user.save();
      res.status(200).send();
    } else next(AppError.unauthorized('Invalid one-time password'));
  }
};
