...
module.exports = class App {
  constructor(port = 3333) {
    ...
    try {
      ...
      this.server = this.app.listen(port, () => {
        logger.debug(`DELIVER API listening on port ${port}`);
      });
  ...
  async setupDatabase() {
    ...
    const database = process.env.MONGODB_DATABASE || 'DELIVER';
    const username = process.env.MONGODB_USERNAME || 'Administrator';
    const password = process.env.MONGODB_PASSWORD || 'manage';
